...
echo ":::: RUN PLANTUML TO GENERATE PNG DIAGRAMS :::"
...
# Run PlantUML to generate PNG diagrams from .puml files in the parent directory
java -Xmx2048m -Dfile.encoding=UTF-8 -DPLANTUML_LIMIT_SIZE=32768 -jar "$(dirname "$(readlink -f "$0")")/plantuml.jar" -tpng -v "./**.puml"

# Revert to the original directory
cd "$currentDir"
echo "Reverted to Original Directory: $currentDir"

echo ":::: PLANTUML PNG GENERATION COMPLETED :::"
