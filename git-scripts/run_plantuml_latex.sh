...
echo ":::: RUN PLANTUML TO GENERATE LATEX DIAGRAMS :::"
...
# Run PlantUML to generate LaTeX diagrams from .puml files
java -Xmx2048m -Dfile.encoding=UTF-8 -DPLANTUML_LIMIT_SIZE=32768 -jar "$PLANTUML_JAR" -tlatex -v "./**.puml"
if [ $? -eq 0 ]; then
    echo "PlantUML diagrams successfully generated."
else
    echo "Error occurred while generating PlantUML diagrams."
    exit 1
fi
